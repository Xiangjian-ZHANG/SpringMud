...
// phase2: rd-port-B or pointer-port
// phase3: rd-port-A
...
#define BR_DS_IF0_PREPARE() \
	ShouldBranch[YI] = !BoolReg[YI];

#define BR_DS_IF1_PREPARE() \
	ShouldBranch[YI] = BoolReg[YI];

#define BACK_BR_DS_EXECUTE(label) \
	if(ShouldBranch[0]) {ShouldBranch[0] = false; goto label;}

#define FWD_BR_DS_EXECUTE(targetPC) \
	if(ShouldBranch[YI]) {ShouldBranch[YI] = false; FwdBrTarget[YI]=targetPC;}
...
#define CALL_PREPARE(returnLabel) \
